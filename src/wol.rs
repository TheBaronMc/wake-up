...
pub fn wake_on_lan(mac_address: &[u8; 12], port: &u16) -> () {
    let address_str = mac_address.iter().map(|b| format!("{:02X}", b))
    .collect::<Vec<String>>()    
    .join(":");
    print!("[WoL] Sending magic packet to host: {}, port: {} ->", address_str, port);
    ...
    print!("[WoL] Magic packet sent to host: {}, port: {} ->", address_str, port);
}
